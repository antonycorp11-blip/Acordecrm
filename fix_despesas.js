...
async function run() {
  ...
  // Find which ones need cloning
  const novasParaInserir = [];
  
  for (const d of fixas) {
    // Check if it already has clones (by description)
    // We can assume if there's only 1 with this description in this month, and we want to create for next 11 months
    // Let's just create 11 months forward based on this one if they don't exist yet.
    ...
    for (let i = 1; i <= 11; i++) {
        ...
        // Check if an expense with the same description and date exists
