...
interface CifrasMusicaisProps {
  onClose?: () => void;
  onGameOverProp?: (score: number) => void;
}

export default function App({ onClose, onGameOverProp }: CifrasMusicaisProps = {}) {
  // --- STATE ---
  ...
  const updatePlayerPosition = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    playerY.current = Math.max(380, Math.min(CANVAS_HEIGHT - 50, clickY));
  };
  ...
  return (
    <div className={isFullscreen ? "fixed inset-0 z-[99999] bg-[#020208] w-screen h-screen flex flex-col font-sans overflow-hidden select-none touch-none overscroll-none" : "w-full max-w-full bg-[#020208] bg-cyber-grid text-white flex flex-col font-sans relative overflow-hidden select-none touch-none overscroll-none"}>
      ...
      {/* MAIN CONTAINER */}
      <main className="flex-1 flex flex-col items-center justify-center p-1 overflow-hidden">
        
        {/* MOBILE CENTER PLAYPORT */}
        <section className="w-full max-w-[480px] mx-auto flex-1 flex flex-col items-center justify-center overflow-hidden">
              ...
              {gameState.isGameOver && (
                ...
                >
                  ...
                  <div className="flex flex-col gap-2 w-full max-w-xs">
                    ...
                    <button
                      onClick={() => {
                        if (onGameOverProp) onGameOverProp(gameState.score);
                        if (onClose) onClose();
