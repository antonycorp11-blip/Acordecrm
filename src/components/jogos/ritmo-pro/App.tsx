...
export const RitmoPro: React.FC<{ onClose: () => void, onGameOver: (score: number) => void }> = ({ onClose, onGameOver }) => {
      ...
      {state === GameState.RANKING && (
        <div className="flex flex-col h-full p-6 animate-in slide-in-from-right duration-300 bg-[#0a0604] py-10">
          <header className="flex justify-between items-center mb-10 px-2">
            ...
            <button onClick={() => { if (scoreRef.current.pts > 0) { onGameOver(scoreRef.current.pts); } onClose(); }} className="bg-white/5 p-3 rounded-2xl text-[10px] font-black text-orange-500 uppercase active:scale-90 transition-all">Sair</button>
          </header>
